...
public class CreateGenerateTypeView {

    public CreateGenerateTypeView(GenerateTypeDialogView generateTypeDialogView) {
        ...
        createBtn.addClickListener(btnEvent -> {
            try {
                ...
                generateTypeDialogView.getRepository()
                        .save(
                                GenerateType.builder().name(name).enumNumber(enumNumber).build()
                        );
            } catch (Exception ex) {
